...
/// failures the ledger may file as an owner that ignores events, how long a
/// refusal by macOS keeps an item's saved slot, and which errors a trigger
/// treats as deferrals rather than failures.
@MainActor
...
struct MoveFailureAttributionTests {
    ...
    @Test("Errors that say nothing about the item defer the trigger", arguments: [
        EventError.inputPauseTimedOut(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .moveSuperseded(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .menuTrackingActive(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .moveEngineBusy(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
    ])
    func deferrals(error: EventError) {
        #expect(MenuBarItemManager.triggerMoveDeferral(for: error) == .deferred)
    }

    @Test("Errors that are verdicts about the item ask the bar", arguments: [
        EventError.dropReverted(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .itemResponseTimeout(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .staleDestination(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .moveTimedOut(.fixture(tag: .appItem(bundleID: "a", title: "b"), windowID: 1)),
        .cannotComplete,
    ])
    func verdicts(error: EventError) {
        #expect(MenuBarItemManager.triggerMoveDeferral(for: error) == nil)
    }
}
