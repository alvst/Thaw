...
struct MovePolicyTests {
    ...
    @Test("The press-release deadline covers a whole attempt and is clamped")
    func pressReleaseDeadline() {
        #expect(MenuBarItemManager.pressReleaseDeadline(for: .milliseconds(100)) == .milliseconds(1500))
        #expect(MenuBarItemManager.pressReleaseDeadline(for: .milliseconds(400)) == .milliseconds(2400))
        #expect(MenuBarItemManager.pressReleaseDeadline(for: .milliseconds(500)) == .seconds(3))
    }
    ...
    @Test("Reasons that prove the item did not land skip the final check", arguments: [
        Policy.StopReason.refusedByMacOS, .ownerUnresponsive, .itemGone, .superseded,
    ])
    ...
    func onlySilenceIsFiled() {
        ...
        let notFiled: [Policy.StopReason] = [.refusedByMacOS, .targetMoved, .targetRetreating, .itemGone, .superseded, .overran, .budgetExhausted, .other]
