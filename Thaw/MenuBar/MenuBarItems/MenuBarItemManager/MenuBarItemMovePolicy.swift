...
extension MenuBarItemManager {
        ...
        nonisolated enum StopReason: Equatable {
            ...
            var deservesFinalLandingCheck: Bool {
                ...
                case .targetMoved, .targetRetreating, .ownerAlwaysSilent, .ownerSilent, .overran, .budgetExhausted, .other:
                    true
                case .refusedByMacOS, .ownerUnresponsive, .itemGone, .superseded:
                    false
                }
            }
            ...
            var isFiledAgainstOwner: Bool {
                ...
                case .ownerUnresponsive, .ownerAlwaysSilent, .ownerSilent:
                    true
                case .refusedByMacOS, .targetMoved, .targetRetreating, .itemGone, .superseded, .overran, .budgetExhausted, .other:
                    false
                }
            }
    ...
    // MARK: - Trigger result mapping

    /// The trigger result a move error maps to without consulting the bar,
    /// or `nil` when the bar has to be asked whether the item is in its
    /// section regardless of the error.
    ///
    /// A deferral is anything that says nothing about the item: the user
    /// was still moving the mouse, a menu was open, the condition changed,
    /// or another move held the bar. Reporting those as failures held the
    /// trigger's action for the backoff period and showed "failed" for a
    /// move that was never attempted.
    static nonisolated func triggerMoveDeferral(for error: EventError) -> TriggerMoveResult? {
        switch error {
        case .inputPauseTimedOut, .moveSuperseded, .menuTrackingActive, .moveEngineBusy:
            .deferred
        case .cannotComplete, .invalidEventSource, .missingMouseLocation, .eventCreationFailure,
             .eventOperationTimeout, .itemNotMovable, .itemResponseTimeout, .missingItemBounds,
             .ownerUnresponsive, .eventWindowMismatch, .staleDestination, .dropReverted, .moveTimedOut:
            nil
        }
    }
}
